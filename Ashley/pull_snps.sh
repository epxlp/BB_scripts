...
#script to extract required SNPs from Biobank PLINK files, merge them(as extracted from different chr separately), recode according to ref allele and make genetic risk score
...
#prerequiste txt files which need to be in home directory (or other but need to specify): list of SNP rs numbers; list of SNP rs number and risk alleles;
...
#mkdir ./biobank_psoriasis/snp_info

#module add languages/R-3.0.2

#R

#source("./scripts/snp_info.R")
